Download official image

# jenkins machine

virt-builder list
virt-builder ubuntu-18.04 -o jenkins.qcow2 --format qcow2 --root-password password:ubuntu --size 10G
cat <<EOF > /tmp/jenkins-script.sh
runuser -l devops -c "mkdir .ssh -m 700"
echo "devops ALL=(ALL) ALL" > /etc/sudoers.d/devops
echo "ssh-rsa  devops@instructor.example.com" > ~devops/.ssh/authorized_keys
echo "ssh-rsa  root@foundation0.ilt.example.com" >> ~devops/.ssh/authorized_keys
rm -f /etc/netplan/*
echo "deb https://pkg.jenkins.io/debian-stable binary/" > /etc/apt/sources.list.d/jenkins.list
apt-get install -y \
    apt-transport-https \
    ca-certificates \
    curl openjdk-8-jdk \
    gnupg-agent \
    software-properties-common
    wget -q -O - https://pkg.jenkins.io/debian/jenkins.io.key | sudo apt-key add -
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -
add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu bionic stable"
apt install docker-ce jenkins -y
#apt install --reinstall openssh-server
EOF

cat <<EOF > ubuntu-network.yaml
network:
  version: 2
  renderer: networkd
  ethernets:
    enp1s0:
      dhcp4: no
      addresses: [192.168.123.11/24]
      gateway4: 192.168.123.1
      nameservers:
        addresses: [192.168.123.1,8.8.8.8]
EOF

virt-customize -a jenkins.qcow2  --run-command 'useradd -m -s /bin/bash devops' --hostname jenkins.example.com --password devops:password:devops \
--firstboot /tmp/gitlab-script.sh --copy-in motd:/etc/ --copy-in ubuntu-network.yaml:/etc/netplan/

virt-install --name jenkins --import --memory 900 --disk jenkins.qcow2 --os-variant ubuntu18.04 --network bridge=virbr0,model=virtio --noautoconsole
----

----

virt-install --name jenkins --import --memory 4096 --vcpus 2 --disk jenkins.qcow2 --os-variant ubuntu18.04 --network bridge=br1,model=virtio --noautoconsole
